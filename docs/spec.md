...
- 编辑条目且密码字段变更时，把旧密码快照（`{ password, at }`）存入顶层 `history` 映射，每条目最多保留最近 5 版（最新在前，与最新相同不重复记录）
- 历史随整体 vault 加密存储（`entries/history/tagDefs/deleted` 一并加密）；**不参与 .vault / CSV 导入导出**（导出只读写 entries）；本地文件同步为整体密文快照，自动包含
- 详情面板「密码历史」区块：显示各版本变更时间与掩码密码，可一键回滚；回滚前自动把当前密码存入历史（防误操作），回滚后更新 `updatedAt`
...
- [x] 支持浏览器扩展（自动填充，MV3，v0.1 实验版已交付，迭代中）
